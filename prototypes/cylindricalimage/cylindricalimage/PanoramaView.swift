...
@objc public enum PanoramaControlMethod: Int {
    case motion
    case touch
}

@objc public enum PanoramaType: Int {
    case cylindrical
    case spherical
}
...
@objc public class PanoramaView: UIView {
    ...
    @objc public var image: UIImage?            { didSet { panoramaType = panoramaTypeForCurrentImage } }
    @objc public var overlayView: UIView?       { didSet { replace(overlayView: oldValue, with: overlayView) } }
    @objc public var panoramaType: PanoramaType = .cylindrical {
        didSet {
            createGeometryNode()
            resetCameraAngles()
        }
    ...
    @objc public var controlMethod: PanoramaControlMethod = .touch {
        didSet {
    ...
    private var panoramaTypeForCurrentImage: PanoramaType {
        if let image = image {
    ...
    private func switchControlMethod(to method: PanoramaControlMethod) {
        ...
        motionManager.deviceMotionUpdateInterval = 0.015625
        motionManager.startDeviceMotionUpdates(using: .xArbitraryZVertical, to: OperationQueue.main, withHandler: {[weak self] (motionData, error) in
            ...
            let R = motionData.attitude.rotationMatrix
            let heading = .pi - atan2(R.m32, R.m31) + .pi/2
            if panoramaView.panoramaType == .cylindrical {
                // Prevent vertical movement in a cylindrical panorama
                panoramaView.cameraNode.eulerAngles = SCNVector3Make(0, Float(-heading), 0)
            } else {
                // Use quaternions when in spherical mode to prevent gimbal lock
                panoramaView.cameraNode.orientation = motionData.orientation()
            }
            panoramaView.reportMovement(CGFloat(heading), panoramaView.xFov.toRadians())
        })
    }
    ...
    func orientation() -> SCNVector4 {
        ...
        switch UIApplication.shared.statusBarOrientation {
            case .landscapeRight:
                ...
                return SCNVector4(x: -q.y, y: q.x, z: q.z, w: q.w)
